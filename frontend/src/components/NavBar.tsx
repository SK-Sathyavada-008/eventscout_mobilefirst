    ...
    <header className="sticky top-0 z-50 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-5 lg:gap-6">
          ...
          <Link href="/explore" className={`text-sm ${isActive("/explore")}`}>
            Explore
          </Link>

          {isAuthenticated ? (
            <>
              <Link href="/saved" className={`text-sm ${isActive("/saved")}`}>
                Saved
              </Link>
              <Link
                href="/preferences"
        ...
        {/* Mobile Right Controls: Notification Bell + Profile Avatar / Login */}
        <div className="flex md:hidden items-center gap-2">
          ...
          {isAuthenticated ? (
            <Link
              ...
              className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-600 text-white font-bold text-xs flex items-center justify-center uppercase shadow-sm border border-indigo-400/30"
              title="Profile"
            >
              {(user?.username || user?.email || "U").slice(0, 2)}
            </Link>
            ...
            <Link
              href="/login"
              className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-500 transition-colors shadow-sm"
            >
              Log in
            </Link>
          )}
